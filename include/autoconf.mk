CONFIG_CMD_FAT=y
CONFIG_BOOTM_NETBSD=y
CONFIG_SYS_MAX_NAND_DEVICE=y
CONFIG_LMB=y
CONFIG_SYS_LOAD_ADDR=0x50000000
CONFIG_DISPLAY_BOARDINFO=y
CONFIG_MMC_SUNXI=y
CONFIG_STACKSIZE="(256 << 10)"
CONFIG_SYS_INIT_RAM_ADDR=0x0
CONFIG_SYS_HELP_CMD_WIDTH=8
CONFIG_NR_DRAM_BANKS=y
CONFIG_BOOTM_LINUX=y
CONFIG_MMC=y
CONFIG_ARM=y
CONFIG_NAND=y
CONFIG_ENV_SIZE="(128 << 10)"
CONFIG_SYS_MALLOC_LEN="(CONFIG_ENV_SIZE + (4 << 20))"
CONFIG_SYS_NO_FLASH=y
CONFIG_INITRD_TAG=y
CONFIG_SYS_MMC_ENV_DEV="mmc_card_no"
CONFIG_CMD_SAVEENV=y
CONFIG_ENV_ADDR="(53 << 20)"
CONFIG_SYS_BAUDRATE_TABLE="{ 9600, 19200, 38400, 57600, 115200 }"
CONFIG_SYS_NS16550_COM3="SUNXI_UART2_BASE"
CONFIG_CMD_EXT2=y
CONFIG_CMD_EXT4=y
CONFIG_ALLWINNER=y
CONFIG_SYS_BOOT_RAMDISK_HIGH=y
CONFIG_SYS_NS16550_CLK="(24000000)"
CONFIG_DOS_PARTITION=y
CONFIG_GZIP=y
CONFIG_SYS_NS16550_COM1="SUNXI_UART0_BASE"
CONFIG_SYS_NS16550_COM2="SUNXI_UART1_BASE"
CONFIG_SYS_NS16550_COM4="SUNXI_UART3_BASE"
CONFIG_SYS_HUSH_PARSER=y
CONFIG_GENERIC_MMC=y
CONFIG_SYS_INIT_SP_OFFSET="(CONFIG_SYS_INIT_RAM_SIZE - GENERATED_GBL_DATA_SIZE)"
CONFIG_EXTRA_ENV_SETTINGS="bootdelay=1\0bootcmd=run setargs_nand boot_normal\0console=ttyS0,115200\0nand_root=/dev/nand3\0loglevel=8\0bootenv=/uEnv.txt\0kernel=/uImage\0scriptbin=/script.bin\0initrd=/uInitrd\0scriptaddr=0x41000000\0scriptbinaddr=0x43000000\0kerneladdr=0x48000000\0initrdaddr=0x50000000\0loadbootenv=mw 41000000 0 10000;ext4load nand 2:0 $scriptaddr /boot${bootenv} || fatload nand 0:0 $scriptaddr ${bootenv};env import 41000000 10000;setenv bootargs console=${console} root=${nand_root} loglevel=${loglevel} ${extraargs}\0loadscriptbin=ext4load nand 2:0 $scriptbinaddr /boot${scriptbin} || fatload nand 0:0 $scriptbinaddr ${scriptbin}\0loadkernel=ext4load nand 2:0 $kerneladdr /boot${kernel} || fatload nand 0:0 $kerneladdr ${kernel}\0loadinitrd=ext4load nand 2:0 $initrdaddr /boot${initrd}\0setmachid=setenv machid 10bb\0setargs_nand=run loadbootenv loadscriptbin loadkernel loadinitrd setmachid\0boot_normal=bootm 48000000 50000000\0boot_recovery=sunxi_flash read 40007800 recovery;boota 40007800\0boot_fastboot=fastboot\0"
CONFIG_SYS_INIT_SP_ADDR="(CONFIG_SYS_INIT_RAM_ADDR + CONFIG_SYS_INIT_SP_OFFSET)"
CONFIG_CMD_IMPORTENV=y
CONFIG_SYS_DEF_EEPROM_ADDR=0
CONFIG_SYS_NS16550_REG_SIZE="(-4)"
CONFIG_SYS_MEMTEST_END="(CONFIG_SYS_SDRAM_BASE + 256<<20)"
CONFIG_FS_EXT4=y
CONFIG_SYS_MEMTEST_START="CONFIG_SYS_SDRAM_BASE"
CONFIG_SUNXI=y
CONFIG_FASTBOOT=y
CONFIG_SYS_BOOT_GET_CMDLINE=y
CONFIG_MMC_LOGICAL_OFFSET="(20 * 1024 * 1024/512)"
CONFIG_CMD_BOOTD=y
CONFIG_MMC_SUNXI_SLOT=2
CONFIG_SYS_TEXT_BASE=0x4A000000
CONFIG_CMD_CRC32=y
CONFIG_SYS_LONGHELP=y
CONFIG_BOOTDELAY=y
CONFIG_SYS_NAND_BASE=0x00
CONFIG_BOOTM_RTEMS=y
CONFIG_SYS_CBSIZE=256
CONFIG_SYS_MONITOR_LEN="(256 << 10)"
CONFIG_FASTBOOT_MMC_NO="mmc_card_no"
CONFIG_NAND_SUNXI=y
CONFIG_CMD_EXPORTENV=y
CONFIG_SYS_NS16550_SERIAL=y
CONFIG_SUNXI_DMA=y
CONFIG_CMD_NAND=y
CONFIG_MMC_SUNXI_USE_DMA=y
CONFIG_CMD_MEMORY=y
CONFIG_SYS_MAXARGS=16
CONFIG_CMD_RUN=y
CONFIG_SYS_PBSIZE=384
CONFIG_SUNXI_ENV_PARTITION="env"
CONFIG_BOARDDIR="board/allwinner/sun7i-evb"
CONFIG_CMDLINE_EDITING=y
CONFIG_BOOTCOMMAND="nand read 50000000 boot;boota 50000000"
CONFIG_SETUP_MEMORY_TAGS=y
CONFIG_ZLIB=y
CONFIG_CMD_GO=y
CONFIG_CMD_BOOTA=y
CONFIG_CMD_BOOTM=y
CONFIG_SYS_NS16550=y
CONFIG_SYS_BARGSIZE="CONFIG_SYS_CBSIZE"
CONFIG_AUTO_COMPLETE=y
CONFIG_SYS_PROMPT_HUSH_PS2="> "
CONFIG_SYS_HZ=1000
CONFIG_CMD_MMC=y
CONFIG_SYS_INIT_RAM_SIZE=0x8000
CONFIG_IDENT_STRING=" Allwinner Technology "
CONFIG_SYS_SDRAM_BASE=0x40000000
CONFIG_BAUDRATE=115200
CONFIG_DISPLAY_CPUINFO=y
CONFIG_CMDLINE_TAG=y
CONFIG_ENV_IS_NOWHERE=y
CONFIG_SUN7I=y
CONFIG_PARTITIONS=y
CONFIG_SYS_PROMPT="unibs#"
CONFIG_STORAGE_EMMC=y
CONFIG_STORAGE_NAND=y
CONFIG_CONS_INDEX=y
